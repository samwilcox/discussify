...
/**
 * Caching class that caches data using a Redis server.
 * 
...
class RedisCache extends \Discussify\Data\Cache\DataCache implements \Discussify\Data\CacheStructure {
    ...
    public function __construct() {
        try {
            ...
            self::$handle->connect(self::settings()->redis_server_host, self::settings()->redis_server_port);
        } catch (\Exception $e) {
            throw new \Discussify\Exceptions\CacheException(\sprintf('Could not connect to the configuted Redis server. Please check your settings. Error: %s', $e));
        }
    }
